...
local timerMarkCD				= mod:NewNextTimer(20.5, 144351, nil, "Healer", nil, 5, nil, DBM_CORE_L.HEALER_ICON)
local timerSelfReflectionCD		= mod:NewNextTimer(25, 144800, nil, nil, nil, 1, nil, nil, nil, 2, 4)
local timerWoundedPrideCD		= mod:NewNextTimer(30, 144358, nil, "Tank", nil, 5, nil, DBM_CORE_L.TANK_ICON)--A tricky on that is based off unit power but with variable timings, but easily workable with an 11, 26 rule
local timerBanishmentCD			= mod:NewNextTimer(37.5, 145215, nil, nil, nil, 3, nil, DBM_CORE_L.HEROIC_ICON)
...
function mod:SPELL_CAST_SUCCESS(args)
	...
	elseif spellId == 146823 and self.Options.SetIconOnFragment then--Banishment cast. Not want to use applied for add mark scheduling
		self:ScanForMobs(72569, 0, 8, 2, nil, 10)
	end
end
