...
local specWarnCharge			= mod:NewSpecialWarningYou(136769, nil, nil, nil, 1, 2, nil, nil, "chargemove")--Maybe add a near warning later. person does have 3.4 seconds to react though and just move out of group.
local yellCharge				= mod:NewShortYell(136769)
local specWarnDoubleSwipe		= mod:NewSpecialWarningDodge(136741, nil, nil, nil, 2, 2, nil, nil, "watchstep")
local specWarnPuncture			= mod:NewSpecialWarningStack(136767, nil, 9, nil, nil, 1, 6, nil, nil, "stackhigh")--9 seems like a good number, we'll start with that. Timing wise the swap typically comes when switching gates though.
local specWarnPunctureOther		= mod:NewSpecialWarningTaunt(136767, nil, nil, nil, 1, 2, nil, nil, "tauntboss")
local specWarnGTFO				= mod:NewSpecialWarningGTFO(136723, nil, nil, nil, 1, 8, nil, nil, "watchfeet")
local specWarnDino				= mod:NewSpecialWarningSwitch(-7086, "-Healer", nil, nil, 1, 2, nil, nil, "killmob")
local specWarnMending			= mod:NewSpecialWarningInterrupt(136797, "-Healer", nil, nil, 1, 2, nil, nil, "kickcast")--High priority interrupt. All dps needs warning because boss heals 1% per second it's not interrupted.
local specWarnOrbofControl		= mod:NewSpecialWarning("specWarnOrbofControl", false, nil, nil, 1, 17, nil, nil, nil, nil, "useitem")--Usually an assigned role for 1-2 people. Do not want someone assigned to interrupts for example hear this and think it's interrupt time. This should be turned on by orb person
local specWarnVenomBolt			= mod:NewSpecialWarningInterrupt(136587, nil, nil, nil, 1, 2, nil, nil, "kickcast")--Can be on for all since it only triggers off target/focus
local specWarnChainLightning	= mod:NewSpecialWarningInterrupt(136480, nil, nil, nil, 1, 2, nil, nil, "kickcast")--Can be on for all since it only triggers off target/focus
local specWarnFireball			= mod:NewSpecialWarningInterrupt(136465, nil, nil, nil, 1, 2, nil, nil, "kickcast")--Can be on for all since it only triggers off target/focus
local specWarnHex				= mod:NewSpecialWarningYou(136512, nil, nil, nil, 1, 17, nil, nil, "debuffyou")
local specWarnJalak				= mod:NewSpecialWarningSwitch(-7087, "Tank", nil, nil, 1, 2, nil, nil, "bigmob")--To pick him up (and maybe dps to switch, depending on strat, "bigmob")
local specWarnDireCall			= mod:NewSpecialWarningCount(137458, nil, nil, nil, 2, 2, 3, nil, "aesoon")--Heroic
local specWarnDireFixate		= mod:NewSpecialWarningRun(140946, nil, nil, nil, 4, 2, 3, nil, "justrun")--Heroic
